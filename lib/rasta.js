...
function fetchRSS(page = 1) {
  return fetch(`${RSS_FEED_URL}&paged=${page}`)
    ...
    .catch(err => log.error('Exception in fetchRSS()', err));
}
...
function snatchReportPhotos(report) {
  return Promise.resolve().then(() => {
    const photosString = report.postImages;
    if (!photosString) return false;
  ...
  }).catch(err => log.error('Exception in snatchReportPhotos', err));
}
...
function fetchLatest(page = 1) {
  ...
  return fetchRSS(page)
    ...
    .catch((error) => {
      log.error('Error in fetchLatest()');
      log.error(error);
      throw error;
    });
}
...
async function sync() {
  ...
  return fetchLatest()
    .catch(err => log.error('Exception in rasta/sync() after calling fetchLatest()', err))
    .then(() => {

 ...
 * @return {Boolean}
 */
function isTimestampFromToday(timestamp, today = moment()) {
  assert.ok(timestamp);
...
function nextFetchTime(latestTimestamp = 1, now = moment()) {
  const eightThirty = moment(now).tz(TIMEZONE).hour(8).minute(30).seconds(0);
  log.debug(eightThirty.format())
  const needToday = !isTimestampFromToday(latestTimestamp);
  log.debug({ needToday });
  const eightThirtyTomorrow = eightThirty.clone().add(1, 'day');
  return needToday ? eightThirty : eightThirtyTomorrow;
}

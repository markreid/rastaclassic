...
const VIDEO_REGEX = /\.(mov|mp4|m4v)/;

const { CACHEBUST, TIMEZONE } = process.env;

const debug = xdebug('rastaclassic:util');
...
function parseReport(post) {
  ...
  props.postVideos = $('a')
    .toArray().filter(a => a.attribs.href && a.attribs.href.match(VIDEO_REGEX))
